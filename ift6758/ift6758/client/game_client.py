...
class DataCleaning:
    ...
    def extract_play(game: dict, play_id) -> dict:
        ...
        if event_name in DataCleaning.considered_event_list:
            ...
            event_team = game.teams['home']['teamName'] if game.teams['home']['team_id'] == event_team_id else \
...
class Game(dict):
    ...
    def calculate_angle(self, positive=False):
        ...
        self._clean['angle_coord_x'] = np.where(self._clean['side'] == 'left', 90 - self._clean['x'],
                                                90 + self._clean['x'])
    ...
    def calculate_time_since_last(self):
        ...
        self._clean['time_full_period_diff'] = self._clean.apply(
            lambda x: (x.period_diff - 1) * 1200 if x.period_diff > 1 else 0, axis=1)
    ...
    def feat_eng_part2(self):
        ...
        self._clean = self._clean[
            self._clean['event_name'].isin(['shot', 'goal', 'blocked-shot', 'missed-shot', 'shot-on-goal'])]

        self._clean['last_event'] = self._clean['event_name'].shift(1)
        self._clean[['last_x', 'last_y']] = self._clean[['x', 'y']].shift(1)

        self._clean['last_distance'] = np.sqrt(
            (self._clean['x'] - self._clean['last_x']) ** 2 + (self._clean['y'] - self._clean['last_y']) ** 2)

        # Seulement un rebond si le dernier évènement est un tir
        self._clean['rebound'] = self._clean['last_event'].apply(
            lambda x: True if x in ['shot', 'blocked-shot', 'missed-shot', 'shot-on-goal'] else False)
    ...
    def current_state(self):
        ...
        period = last_event['period'].values[0]
        # time_remaining = last_event['periodTimeRemaining']
        time_remaining = pd.to_timedelta('00:' + last_event['periodTimeRemaining']).dt.total_seconds().values[0]
    ...
    def updated_clean_game(self):
        ...
        return game_clean.merge(old_game_clean, indicator=True, how='left').loc[
